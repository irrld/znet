...
/**
 * @brief Listens on one address and drives every accepted session on its
 *        worker threads, reporting through the event callback.
 */
class Server : public Interface {
 public:
  ...
  /** @brief Binds the configured address. Not thread-safe. InvalidAddress,
   *         CannotCreateSocket or CannotBind say what failed. */
  Result Bind() override;

  /** @brief Starts accepting connections on a thread of its own and returns
   *         at once. Not thread-safe. AlreadyListening if it already is. */
  Result Listen();

  /** @brief Blocks until the server's thread has finished, which is after
   *         Stop() or a fatal error. Thread-safe. */
  void Wait() override;

  /** @brief Stops listening and tears every session down. Thread-safe.
   *         AlreadyStopped if it was not listening. */
  Result Stop();

  /** @brief How often the workers service their sessions, clamped to at
   *         least 1. Thread-safe. */
