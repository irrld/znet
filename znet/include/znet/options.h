...
//   config.child_options.zdt.max_messages_in_flight = 128;    // every accepted session
//
...
struct ZDTOptions {
   ...
   * control; the congestion window is max_datagrams_in_flight.
   *
   ...
   */
  int max_messages_in_flight = 4096;
  ...
  /** @brief Established connections a server will hold. Zero means unlimited,
   * as with ServerOptions::max_connections. */
  int max_connections = 4096;
  /** @brief Handshake messages accepted per source address per second.
   * The transport-level cousin of ServerOptions::max_attempts_per_source,
